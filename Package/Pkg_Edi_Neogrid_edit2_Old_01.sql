...
select
 ...
 round(sum(((V.QTDITEM - V.QTDDEVOLITEM) / K.QTDEMBALAGEM) *
     (nvl(a.propqtdprodutobase, 1))),3)* 1000 as QUANTIDADE,
 CASE
   WHEN v.codgeraloper in (205) then
    'S'
   else
    'N'
 END AS BONIFICACAO,
  ROUND(fC5_Divide(round(sum(V.VLRITEM),2),
                  round(sum(round((V.QTDITEM - V.QTDDEVOLITEM) /
                                  K.QTDEMBALAGEM,
                                  6) * (nvl(a.propqtdprodutobase, 1))),
                        2)) / K.QTDEMBALAGEM,
       2) * 100 as VLRUNITARIO,
 \*ROUND(fC5_Divide(sum((round(V.VLRITEM, 2) - VLRICMSST - VLRFCPST) -
                (round(V.VLRDEVOLITEM, 2) -
                (0 + VLRDEVOLICMSST + DVLRFCPST))),
            round(sum(round((V.QTDITEM - V.QTDDEVOLITEM) / K.QTDEMBALAGEM, 6) *
                      (nvl(a.propqtdprodutobase, 1))),
                  4))/ K.QTDEMBALAGEM,2) * 100 as VLRUNITARIO,*\

 sum((round(V.VLRITEM, 2)))*100 as VLRBRUTO,
 ROUND(sum((round(V.VLRITEM, 2) - VLRICMSST - VLRFCPST) -
     (round(V.VLRDEVOLITEM, 2) - (0 + VLRDEVOLICMSST + DVLRFCPST))),2)*100 as VLRLIQUIDO,

 \*fC5_Divide(
 sum( ( round( V.VLRITEM, 2 ) ) ),
 round(sum( V.QTDITEM / K.QTDEMBALAGEM * ( nvl( a.propqtdprodutobase, 1 )) ),4)
 )
 as VLRUNITARIOBRT,*\
 ...
 where D.SEQFAMILIA = A.SEQFAMILIA
   ...
   and V.DTAVDA Between Pddtainicial And
                               Pddtafinal
...
V.NRODOCTO   AS NUMERODF,
 ...
 V.seqproduto AS CODIGOPROD,
 A.DESCCOMPLETA AS DESCPRODUTO,
 round(sum(  V.QTDDEVOLITEM / K.QTDEMBALAGEM * ( nvl( a.propqtdprodutobase, 1 )) ),3) * 1000
as QUANTIDADE,
 CASE
   WHEN v.codgeraloper in (205) then
    'S'
...
ROUND(fC5_Divide(
sum( (  ( round( V.VLRDEVOLITEM, 2 ) ) ) ),
round(sum(  V.QTDDEVOLITEM / K.QTDEMBALAGEM * ( nvl( a.propqtdprodutobase, 1 )) ),4)
) / K.QTDEMBALAGEM ,2)*100
as VLRUNITARIO,

ROUND(sum( ( ( round( V.VLRDEVOLITEM, 2 ) ) ) ),2)*100
as VLRVENDABRUTA,
 ROUND(sum( V.VLRDEVOLITEM ),2)*100
as  VLRVENDALIQUIDA,
...
from MAXV_ABCDISTRIBBASE V, MRL_CUSTODIAFAM Y, MAP_PRODUTO A, MAP_PRODUTO PB, MAP_FAMDIVISAO D, MAP_FAMEMBALAGEM K, MAX_EMPRESA E, MAX_DIVISAO DV, MAP_PRODACRESCCUSTORELAC PR, MRLV_DESCONTOREGRA RE where D.SEQFAMILIA = A.SEQFAMILIA
and D.NRODIVISAO = V.NRODIVISAO
and V.SEQPRODUTO = A.SEQPRODUTO
and V.SEQPRODUTOCUSTO = PB.SEQPRODUTO
and V.NROEMPRESA = vEmpresas.CodEmpresa
 ...
 group by V.NRODOCTO, V.SERIEDOCTO, V.seqproduto, v.codgeraloper, A.DESCCOMPLETA,K.QTDEMBALAGEM
       ...
       --    Vslinha := Vslinha || ';';
        ...
        --   Vslinha := Vslinha || ';';
         ...
         --  Vslinha := Vslinha || ';';
          -- Filler
          Vslinha := Vslinha || Rpad(' ', 27, ' ');
         --  Vslinha := Vslinha || ';';
          -- Tipo da Nota Fiscal
          Vslinha := Vslinha || Lpad(Vtvenda.Tiponotafiscal, 2, '0');
          -- Vslinha := Vslinha || ';';
          -- Filler
          Vslinha := Vslinha || Rpad(Vtvenda.Vlrdesconto, 8, '0');
          -- Vslinha := Vslinha || ';';
          -- Descrição do Produto
...
SELECT
  V.NRODOCTO   AS NUMERODF,
   ...
   A.DESCCOMPLETA AS DESCPRODUTO,
   round(
    sum(((V.QTDITEM - V.QTDDEVOLITEM)) * (nvl(a.propqtdprodutobase, 1))),3
  )* 1000
  as QUANTIDADE,
   CASE
  ...
  AS BONIFICACAO,
   ROUND(
    fC5_Divide(round(sum(V.VLRITEM),2),round(sum(round((V.QTDITEM - V.QTDDEVOLITEM) /K.QTDEMBALAGEM,6) * (nvl(a.propqtdprodutobase, 1))),2)) / K.QTDEMBALAGEM,2
  ) * 100
  as VLRUNITARIO,
  ...
  /* Incluido por Hilson Santos em 16/11/2022 os CGO de devoluções no CASE*/
   CASE
 ...
 where D.SEQFAMILIA = A.SEQFAMILIA
   ...
   AND V.NROEMPRESA = RE.NROEMPRESA(+)
   /* Incluido por Hilson Santos em 16/11/2022 os CGO de devoluções no CASE*/
   AND v.codgeraloper in (220, 201, 307, 314, 598, 205, 102, 133, 173, 177, 188, 251, 401, 402, 567, 581, 708)
   and Y.SEQFAMILIA = PB.SEQFAMILIA
  -- AND V.seqproduto IN(37474,39484,39415,38929,39)
      AND V.seqproduto IN (SELECT DISTINCT  D.SEQPRODUTO
                             FROM MAF_FORNECEDI A
