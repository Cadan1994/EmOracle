CREATE OR REPLACE PACKAGE BODY IMPLANTACAO.PKG_FIAGRUPATITAUTO_cadan IS
  ...
  //-----------------------------------------------------------------//
  // O objetivo dessa alteração, foi para o agrupamento  dos títutos //
  // referente as compras realizadas pelo ECOMMERCE, onde esse pro - //
  // cesso dar baixa nos títulos dos clientes e cria um novo em nome //
  // da getnet com o código do cliente 37266                         //
  /////////////////////////////////////////////////////////////////////
  //-----------------------------------------------------------------//
  // Objeto modificado por Hilson Santos em 19/12/2023               //
  //-----------------------------------------------------------------//
  // Alterado a linha 199 desse script                               //
  /////////////////////////////////////////////////////////////////////
  //-----------------------------------------------------------------//
  // Objeto modificado por Hilson Santos em 05/01/2024               //
  //-----------------------------------------------------------------//
  // Alterado a linha 199 desse script                               //
  /////////////////////////////////////////////////////////////////////
  //-----------------------------------------------------------------//
  // Objeto modificado por Hilson Santos em 08/01/2024               //
  //-----------------------------------------------------------------//
  // Alterado a linha 200 desse script                               //
  /////////////////////////////////////////////////////////////////////
  */
	...
	PROCEDURE FIP_AGRUPAUTTITULOS(psUsuario    IN GE_USUARIO.CODUSUARIO%TYPE, -- Usuário usado para registro das transações e log, em caso de nulo será usado o nome AUTOMATICO
		...
		vtnNroPedido       MAD_PEDVENDA.NROPEDVENDA%TYPE;
		...
		vnVlrTitulo   		 FI_TITULO.VLRNOMINAL%TYPE;
	...
	BEGIN
		...
		IF vsTipoData = 'E' THEN
			vsSqlAnd := 'AND c.dtaemissao BETWEEN ''' || vdDtaInicial ||
									''' AND ''' || vdDtaFinal || ''' ';
		ELSIF vsTipoData = 'V' THEN
			vsSqlAnd := 'AND c.dtavencimento BETWEEN ''' || vdDtaInicial ||
									''' AND ''' || vdDtaFinal || ''' ';
		END IF;

		/* Monta a query de consulta */
		/* Alterado em 14-02/2024 por Hilson Santos */
		vsSqlConsultatitagr := 'SELECT
															 ...
															 c.seqpessoa,
															 c.codespecie,
															 /* Alterado de a.dtainclusão "MAD_PEDVENDA" para c.dtainclusao "FI_TITULO" para d.dtamovimento "MAD_PEDVENDANSU" */
															 NVL(d.dtamovimento,c.dtainclusao) dtamovimento,
															 /* Incluido esse campo referente ao código do pedido "MAD_PEDVENDA" */
															 d.nropedvenda,
															 d.rede,
															 d.bandeira,
															 d.nsu
														...
														/* Alterado em 09-02-2024 por Hilson Santos */
														/* Incluido o relacionamento NROSEGMENTO e alterado NROPEDVEDA para NROCARGA* entre as tabelas MAD_PEDVENDA e MFL_DOCTOFISCAL */
														INNER JOIN implantacao.mfl_doctofiscal b ON b.nrosegmento = a.nrosegmento AND b.nrocarga IN a.nrocarga
														INNER JOIN implantacao.fi_titulo c ON c.nrotitulo = b.numerodf AND c.situacao != ''C''
													  ...
													  vsSqlAnd || '
														--GROUP BY c.nroempresamae,c.nroempresa,c.seqpessoa,c.codespecie,c.dtainclusao,d.dtamovimento,d.nropedvenda,d.rede,d.bandeira,d.nsu
		...
		LOOP
			...
			INTO vtnNroEmpresaMae, vtnNroEmpresa, vtnSeqPessoa, vtsCodEspecie, vtdDtaVencimento, vtnNroPedido, vtnRede, vtnBandeira, vtsNsu;
			...
			vsSqlConsultatitind := 'SELECT
																 ...
																 INNER JOIN implantacao.mfl_doctofiscal b ON b.nrosegmento = a.nrosegmento AND b.nrocarga IN a.nrocarga
																 INNER JOIN implantacao.fi_titulo c ON c.nrotitulo = b.numerodf AND c.situacao != ''C''
																 INNER JOIN implantacao.fi_compltitulo d ON d.seqtitulo = c.seqtitulo ' ||
														 vsSqlWhere || vsSqlAnd;
			...
			LOOP
				...
				/* Cuidado ao alterar a ordem das variáveis do into, eles devem estar na mesma ordem das colunas montadas */
				INTO vnSeqTitulo, vnSeqTitBaixa, vnNroTituloNew, vtnvlroriginal, vtnVlrDescFin, vtnVlrDescContrato;
				...
				vbOk := FIF_BAIXATITULOS(vnSeqTitulo,
																 vnCodOpeQuitDir,
																 vnCodOpeMultaDir,
																 vnCodOpeJurosDir,
																 vnCodOpeDescDir,
				...
				IF vbOk THEN
					...
					UPDATE FI_TITULO A
						 SET A.ABERTOQUITADO = 'Q', A.DTAQUITACAO = vdDtaExecucao
					 WHERE A.SEQTITULO = vnSeqTitBaixa;
				END IF;
						 
				vnVlrTotal := vnVlrTotal + vtnvlroriginal;
			...
			IF vbOk THEN
				...
				IF vbOk THEN
					...
					BEGIN
						...
						PKG_FINANCEIRO.FIP_INCLUITITULO(37266,
																						vnNroTitulo,
																						vsSeriePadrao,
																						1,
																						1,
						...
						IF vsTransfCartTitGer = 'S' AND vsLancaOpTxAdm = 'N' THEN
							...
							INSERT INTO FI_TITULONSU
								(FI_TITULONSU.SEQTITULONSU,
	...
	BEGIN
		...
		INSERT INTO FIX_SEQTITULOS
			...
			SELECT pnNroProcessoExe, TO_NUMBER(COLUMN_VALUE)
				FROM TABLE(CAST(C5_COMPLEXIN.C5INTABLECLOB(pnSeqTitulo) AS
												C5INCLOBTABLE));
	...
	BEGIN
		...
		IF psParamUsaDesconto = 'T' THEN
			...
			ELSIF psParamDataLimiteDesc = 'M' THEN
				...
				SELECT MIN(TO_DATE(COLUMN_VALUE, 'DD/MM/YYYY')) +
							 ROUND(TO_NUMBER(MAX(TO_DATE(COLUMN_VALUE, 'DD/MM/YYYY')) -
															 MIN(TO_DATE(COLUMN_VALUE, 'DD/MM/YYYY'))) / 2,
										 0)
					INTO vdRetorno
					FROM TABLE(CAST(C5_COMPLEXIN.C5INTABLECLOB(psDtaLimDescFin) AS
													C5INCLOBTABLE));
