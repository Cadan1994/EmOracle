CREATE OR REPLACE PACKAGE IMPLANTACAO.PKG_FIAGRUPATITAUTO IS
...
END PKG_FIAGRUPATITAUTO;
/
CREATE OR REPLACE PACKAGE BODY IMPLANTACAO.PKG_FIAGRUPATITAUTO IS
-- Procedure principal de processamento
PROCEDURE FIP_AGRUPAUTTITULOS( psUsuario           IN GE_USUARIO.CODUSUARIO%TYPE, -- Usuário usado para registro das transações e log, em caso de nulo será usado o nome AUTOMATICO
                               pdDtaInicial        IN FI_TITULO.DTAEMISSAO%TYPE,  -- Data inicial para busca dos títulos, caso não seja informado serão filtrados todas as datas
                               pdDtaFinal          IN FI_TITULO.DTAEMISSAO%TYPE,  -- Data final para busca dos títulos, caso não seja informado serão filtrados todas as datas
...
BEGIN
     ...
     vdDtaFinal    := NULL;
     vsTipoData    := NULL;
     vsUsuario     := NVL(psUsuario,'AUTOMATICO');
     ...
     vsSqlAgrupamento := vsSqlAgrupamento || ' FI_TITULO.OBRIGDIREITO,
                                       FI_TITULO.SEQPESSOA, ';
     IF vsAgrupaEspecieDestino = 'S' THEN
        vsSqlAgrupamento := vsSqlAgrupamento || ' FI_ESPECIE.CODESPECIEAGRUP, ';
     ELSE
        vsSqlAgrupamento := vsSqlAgrupamento || ' FI_ESPECIE.CODESPECIE, ';
     END IF;
     -- Fixo ( manter nesta posição )
     vsSqlAgrupamento := vsSqlAgrupamento || ' PKG_FIAGRUPATITAUTO.FIF_DTAVENCIMENTO('''||vdDtaExecucao||''' , FI_TITULO.DTAVENCIMENTO , FI_TITULO.SEQPESSOA , FI_TITULO.OBRIGDIREITO ) ';
     -- Monta as colunas
     -- Cuidado ao alterar a ordem das colunas, eles devem estar na mesma ordem das variáveis do into
     vsSqlColunas := ' SELECT ' || vsSqlAgrupamento || ', NVL(COUNT(1),0)                                                                                               QTDTITULOS,
     ...
     vsSqlWhere := ' FROM  FI_TITULO,
                     ...
                     AND   NOT EXISTS ( SELECT 1
                                        FROM   FI_DEBAUT
                                        WHERE  FI_DEBAUT.SEQTITULO = FI_TITULO.SEQTITULO
                                        AND    FI_DEBAUT.SITUACAO = ''A''
                                        AND    FI_DEBAUT.CODOCORRENCIA = 1 )
                     AND   NOT EXISTS ( SELECT 1
                                        FROM   FI_ACCARGA
                                        WHERE  FI_ACCARGA.SEQTITULO = FI_TITULO.SEQTITULO )
                     AND   ( ( EXISTS ( SELECT 1
                                        FROM   FI_FORNECEDOR
                                        WHERE  FI_FORNECEDOR.SEQPESSOA = FI_TITULO.SEQPESSOA
                                        AND    FI_FORNECEDOR.INDAGRUPATITULO = ''S'' )
                               AND FI_TITULO.OBRIGDIREITO = ''O''
                             )
                             OR
                             ( EXISTS ( SELECT 1
                                        FROM   FI_CLIENTE
                                        WHERE  FI_CLIENTE.SEQPESSOA = FI_TITULO.SEQPESSOA
                                        AND    FI_CLIENTE.INDAGRUPATITULO = ''S'' )
  ...
  /* Filtro por periodo de apuração
   ...
   '
    AND (NVL( CASE WHEN FI_TITULO.OBRIGDIREITO = ''O'' THEN
               (SELECT A.INDUSAPERIODOAGRUPAUTO FROM FI_FORNECEDOR A WHERE A.SEQPESSOA = FI_TITULO.SEQPESSOA)
              ELSE
         ...
         EXISTS (SELECT A.SEQPERIODO
                  ...
                  WHERE A.SEQPESSOA = FI_TITULO.SEQPESSOA
                    AND A.OBRIGDIREITO = FI_TITULO.OBRIGDIREITO
                    AND PKG_FIAGRUPATITAUTO.FIF_MONTADATA(A.DIAEXE, A.MESEXE, A.MESEXE, '||vnAnoExe||', NULL)
                        = '''||vdDtaExecucao||'''
                    AND FI_TITULO.DTAEMISSAO BETWEEN
                        PKG_FIAGRUPATITAUTO.FIF_MONTADATA(A.DIAINI, A.MESINI, A.MESEXE, '||vnAnoExe||', ''P'')
                        AND
                        PKG_FIAGRUPATITAUTO.FIF_MONTADATA(A.DIAFIM, A.MESFIM, A.MESEXE, '||vnAnoExe||', ''P'')
                    ))';
     --
     IF vsPermAgrupTitVencido = 'N' THEN
        vsSqlWhere := vsSqlWhere || ' AND FI_TITULO.DTAPROGRAMADA >= TRUNC(SYSDATE) ' ;
     END IF;
     --
     IF vsPermAgrupTitAgrupado = 'N' THEN
        vsSqlWhere := vsSqlWhere || ' AND NVL(FI_TITULO.APPORIGEM,''X'') != ''AGRUPTITAUTO'' ';
     END IF;
     --
     IF vsTipoData = 'E' THEN
        vsSqlWhere := vsSqlWhere || ' AND FI_TITULO.DTAEMISSAO >= ''' || vdDtaInicial || '''
                                      AND FI_TITULO.DTAEMISSAO <= ''' || vdDtaFinal || '''';
     ELSIF vsTipoData = 'V' THEN
        vsSqlWhere := vsSqlWhere || ' AND FI_TITULO.DTAVENCIMENTO >= ''' || vdDtaInicial || '''
                                      AND FI_TITULO.DTAVENCIMENTO <= ''' || vdDtaFinal || '''';
     END IF;
     -- Monta o agrupamento
     vsSqlGroupBy := ' GROUP BY ' || vsSqlAgrupamento;
     vsSqlGroupBy := vsSqlGroupBy || ' HAVING ( ( COUNT(1) > 1 ) OR ( MAX(FI_ESPECIE.INDGERATITAGRUPAUT) = ''S'' ) )';
     -- Monta a ordenação
     vsSqlOrderBy := ' ORDER BY ' || vsSqlAgrupamento;
     -- Monta a consulta
     vsSqlConsulta := vsSqlColunas || vsSqlWhere || vsSqlGroupBy || vsSqlOrderBy;
     -- Executa o loop
     ...
     LOOP
         ...
         -- Log de registro
         FIP_GRAVALOG( 'Inicio do agrupamento dos títulos da sequência: ' || SUBSTR(vtcInSeqTitulos,1,450),
                       NULL,
                       vnNroProcessoExe,
                       vnNroProcessoTit,
                       'I',
         ...
         IF vbOk THEN
            ...
            IF vnVlrTotal <= 0 THEN
               vbOk := FALSE;
               FIP_GRAVALOG( 'O valor do novo título ' || vnNroTitulo || ' deve ser maior que zero. ( Valor ' || PKG_FINANCEIRO.FIF_FORMATAVALOR( vnVlrTotal ) || ' ) : ' || SQLERRM || ' - ' || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
                             vnSeqTitNovo,
                             vnNroProcessoExe,
                             vnNroProcessoTit,
                             'E',
            ...
            IF vbOk THEN
               ...
               INTO   vsIndReplicaComprador,
                      vsLancaOpTxAdm
               FROM   FI_PARAMETRO A
               WHERE  A.NROEMPRESA = vnNroEmpresa;
               IF vsTransfCartTitGer = 'S' AND vsLancaOpTxAdm = 'N' THEN
                 UPDATE FI_TITULONSU
		                SET SEQTITULO = vnSeqTitNovo,
                        FI_TITULONSU.ORIGEM = 'AGRUPTITAUTO',
                        FI_TITULONSU.ORIGEMATUALIZACAO = NULL
		              WHERE FI_TITULONSU.SEQTITULO IN
                                    ( SELECT TO_NUMBER(COLUMN_VALUE)
                                        FROM TABLE( CAST( C5_COMPLEXIN.C5INTABLECLOB( vtcInSeqTitulos ) AS C5INCLOBTABLE ) ) )
                    AND NOT EXISTS ( SELECT 1
               ...
               FIP_GRAVALOG( 'Inclusão do título ' || vnNroTitulo || ' no valor de ' || PKG_FINANCEIRO.FIF_FORMATAVALOR( vnVlrTotal ) || ' para a espécie ' || vsCodEspecieNovoTit || ' efetuado com sucesso!',
                              vnSeqTitNovo,
                              vnNroProcessoExe,
                              vnNroProcessoTit,
                              'R',
               ...
               EXCEPTION
                        WHEN OTHERS THEN
                             vbOk := FALSE;
                             FIP_GRAVALOG( 'Erro ao inserir o título ' || vnNroTitulo || ' no valor de ' || PKG_FINANCEIRO.FIF_FORMATAVALOR( vnVlrTotal ) || ' para a espécie ' || vsCodEspecieNovoTit || ' : ' || SQLERRM || ' - ' || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
                                           vnSeqTitNovo,
                                           vnNroProcessoExe,
                                           vnNroProcessoTit,
                                           'E',
         ...
         IF vbOk THEN
            ...
            EXCEPTION
                     WHEN OTHERS THEN
                          vbOk := FALSE;
                          FIP_GRAVALOG( 'Erro ao executar processo de comissão para o título ' || vnNroTitulo || ' : ' || SQLERRM || ' - ' || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
                                        vnSeqTitNovo,
                                        vnNroProcessoExe,
                                        vnNroProcessoTit,
                                        'E',
         ...
         IF vbOk THEN
            ...
            COMMIT;
            -- Log de registro
            FIP_GRAVALOG( 'Agrupamento finalizado com sucesso, processo: ' || vnNroProcessoTit,
                          NULL,
                          vnNroProcessoExe,
                          vnNroProcessoTit,
                          'I',
         ...
         ELSE
            ...
            ROLLBACK;
            -- Log de registro
            FIP_GRAVALOG( 'Houve erros no agrupamentos dos títulos do processo: ' || vnNroProcessoTit || ' . Os lançamentos gerados serão desfeitos!',
                          NULL,
                          vnNroProcessoExe,
                          vnNroProcessoTit,
                          'I',
     ...
     EXCEPTION
             WHEN OTHERS THEN
                  ROLLBACK;
                  --
                  FIP_GRAVALOG( 'Ocorreu um erro ao executar o agrupamento automático referente ao processo : ' || vnNroProcessoExe || ' . Os lançamentos gerados serão desfeitos! : ' || SQLERRM || ' - ' || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
                                NULL,
                                vnNroProcessoExe,
                                vnNroProcessoTit,
                                'E',
...
BEGIN
     ...
     --
     INSERT INTO FIX_SEQTITULOS( NROPROCESSO,
                                 SEQTITULO )
     SELECT pnNroProcessoExe,
            TO_NUMBER(COLUMN_VALUE)
     FROM   TABLE( CAST( C5_COMPLEXIN.C5INTABLECLOB( psTitulos ) AS C5INCLOBTABLE ) );
        ...
        LOOP
            ...
            IF vbOk THEN
               IF psUtilizaDesc = 'Q' THEN
                  ...
                  EXCEPTION
                           WHEN OTHERS THEN
                                vbOk := FALSE;
                                FIP_GRAVALOG( 'Erro no cálculo do desconto do título ' || vtQuitaTit.TITULO || ' sequencial do título ' || vtQuitaTit.SEQTITULO || ' : ' || SQLERRM || ' - ' || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
                                              vtQuitaTit.SEQTITULO,
                                              pnNroProcessoExe,
                                              pnNroProcessoTit,
                                              'E',
            ...
            IF vbOk THEN
               IF psMultaJuros = 'S' THEN
                  ...
                  EXCEPTION
                           WHEN OTHERS THEN
                                vbOk := FALSE;
                                FIP_GRAVALOG( 'Erro no cálculo de multa e juros do título ' || vtQuitaTit.TITULO || ' sequencial do título ' || vtQuitaTit.SEQTITULO || ' : ' || SQLERRM || ' - ' || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
                                              vtQuitaTit.SEQTITULO,
                                              pnNroProcessoExe,
                                              pnNroProcessoTit,
                                              'E',
            ...
            IF vbOk THEN
               ...
               IF vnValor > 0 THEN
                  ...
                  -- Registra o lançamento
                  IF vbOk THEN
                     FIP_GRAVALOG( 'Operação de quitação ' || pnCodOpeQuit || ' no valor de ' || PKG_FINANCEIRO.FIF_FORMATAVALOR( vnValor ) || ' para o título ' || vtQuitaTit.TITULO || ' lançada com sucesso!',
                                    vtQuitaTit.SEQTITULO,
                                    pnNroProcessoExe,
                                    pnNroProcessoTit,
                                    'R',
            ...
            IF vbOk THEN
               IF vnVlrMulta > 0 THEN
                  ...
                  -- Registra o lançamento
                  IF vbOk THEN
                     FIP_GRAVALOG( 'Operação de multa ' || pnCodOpeMulta || ' no valor de ' || PKG_FINANCEIRO.FIF_FORMATAVALOR( vnVlrMulta ) || ' para o título ' || vtQuitaTit.TITULO || ' lançada com sucesso!',
                                    vtQuitaTit.SEQTITULO,
                                    pnNroProcessoExe,
                                    pnNroProcessoTit,
                                    'R',
            ...
            IF vbOk THEN
               IF vnVlrJuros > 0 THEN
                  ...
                  -- Registra o lançamento
                  IF vbOk THEN
                     FIP_GRAVALOG( 'Operação de juros ' || pnCodOpeJuros || ' no valor de ' || PKG_FINANCEIRO.FIF_FORMATAVALOR( vnVlrJuros ) || ' para o título ' || vtQuitaTit.TITULO || ' lançada com sucesso!',
                                    vtQuitaTit.SEQTITULO,
                                    pnNroProcessoExe,
                                    pnNroProcessoTit,
                                    'R',
            ...
            IF vbOk THEN
               IF vnVlrDesconto > 0 THEN
                  ...
                  -- Registra o lançamento
                  IF vbOk THEN
                     FIP_GRAVALOG( 'Operação de desconto ' || pnCodOpeDesc || ' no valor de ' || PKG_FINANCEIRO.FIF_FORMATAVALOR( vnVlrDesconto ) || ' para o título ' || vtQuitaTit.TITULO || ' lançada com sucesso!',
                                    vtQuitaTit.SEQTITULO,
                                    pnNroProcessoExe,
                                    pnNroProcessoTit,
                                    'R',
               ...
               IF vbOK THEN
                 ...
                 IF vnVlrDescontoDet > 0 THEN
                    FOR vtDescDet IN (SELECT A.SEQTITDESCONTO, A.CODOPERACAO,
                                      ...
                                      AND    (A.DTALIMITE IS NULL OR
                                              FIF_DATAUTIL(A.DTALIMITE, B.SEQPESSOA, 0, 0, 'P', B.NROEMPRESA) >= TRUNC(SYSDATE) ))
                    LOOP
                         ...
                         -- Registra o lançamento
                         IF vbOk THEN
                            FIP_GRAVALOG( 'Operação de desconto detalhado ' || vtDescDet.CODOPERACAO || ' no valor de ' || PKG_FINANCEIRO.FIF_FORMATAVALOR( vtDescDet.VLRDISPONIVEL ) || ' para o título ' || vtQuitaTit.TITULO || ' lançada com sucesso!',
                                          vtQuitaTit.SEQTITULO,
                                          pnNroProcessoExe,
                                          pnNroProcessoTit,
                                          'R',
            ...
            IF vbOk THEN
               FIP_GRAVALOG( 'Baixa do título ' || vtQuitaTit.TITULO || ' finalizada.',
                             vtQuitaTit.SEQTITULO,
                             pnNroProcessoExe,
                             pnNroProcessoTit,
                             'R',
            ...
            ELSE
               FIP_GRAVALOG( 'Baixa do título ' || vtQuitaTit.TITULO || ' sequencial do título ' || vtQuitaTit.SEQTITULO || ' não realizada.',
                             vtQuitaTit.SEQTITULO,
                             pnNroProcessoExe,
                             pnNroProcessoTit,
                             'R',
...
BEGIN
     ...
     EXCEPTION
              WHEN OTHERS THEN
                   vbOk := FALSE;
                   FIP_GRAVALOG( 'Erro no lançamento da operação ' || pnCodOperacao || ' no valor de ' || PKG_FINANCEIRO.FIF_FORMATAVALOR( pnValor ) || ' para o sequencial do título ' || pnSeqTitulo || ' : ' || SQLERRM || ' - ' || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,
                                 pnSeqTitulo,
                                 pnNroProcessoExe,
                                 pnNroProcessoTit,
                                 'E',
...
BEGIN
     ...
     IF psParamUsaDesconto = 'T' THEN
        ...
        ELSIF psParamDataLimiteDesc = 'M' THEN
           ...
           SELECT MIN(TO_DATE(COLUMN_VALUE)) + ROUND(TO_NUMBER(MAX(TO_DATE(COLUMN_VALUE)) - MIN(TO_DATE(COLUMN_VALUE))) / 2,0)
           INTO   vdRetorno
           FROM   TABLE( CAST( C5_COMPLEXIN.C5INTABLECLOB( psDtaLimDescFin ) AS C5INCLOBTABLE ) );
        ELSE
           -- Data de vencimento
