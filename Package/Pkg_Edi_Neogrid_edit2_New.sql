...
SERÃO CONSISTIDOS EM CONJUNTO COM O CGO INFORMADO NO PARÂMETRO DA EMPRESA. INFORMAR OS CGOS SEPARADOS POR VIRGULA. DEFAULT: 0',
  ...
  Begin
    if psVersaoLayout = '4' or psVersaoLayout = '04' OR
       ...
       For vtVendedor in (
			SELECT
				b.nomerazao AS NomeRazaoRepres,
				CASE
				WHEN LENGTH(b.nrocgccpf) = 9
				THEN LPAD(b.nrocgccpf,9,0)||LPAD(b.digcgccpf,2,0)
				ELSE LPAD(a.nrorepresentante,11,0)
				END
				AS NroRepresentante,
				d.Nomerazao AS Nomerazaosup,
				LPAD(d.nrocgccpf,9,0)||LPAD(d.digcgccpf,2,0) AS SeqPessoaSup,
				f.nomerazao AS NomeGerente,
				LPAD(f.nrocgccpf,9,0)||LPAD(f.digcgccpf,2,0) AS NroGerente,
                NVL(a.indpartclubeitt, 'N') AS Indpartclubeitt,
                CASE
                WHEN NVL(a.indpartclubeitt, 'N') = 'N'
				THEN '20501231'
                ELSE NVL(TO_CHAR(a.dtavigenciaclubeitt,'yyyymmdd'),'20501231')
                END AS Dtavegenciaclubeitt
			FROM implantacao.mad_representante a
			INNER JOIN implantacao.ge_pessoa b ON b.seqpessoa = a.seqpessoa AND b.nrocgccpf NOT IN ('000000000','111111111','222222222','333333333','444444444','555555555','666666666','777777777','888888888','999999999')
			INNER JOIN implantacao.mad_equipe c ON c.nroequipe = a.nroequipe
			LEFT  JOIN implantacao.ge_pessoa d ON d.seqpessoa = c.seqpessoa
			LEFT  JOIN implantacao.mad_equipe e ON e.nroequipe = c.nroequipesuperior
			LEFT  JOIN implantacao.ge_pessoa f ON f.seqpessoa = e.seqpessoa
			WHERE 1=1
	   ...
	   Loop
			...
			--GERENTE
			Vslinha := Vslinha || Rpad(nvl(Vtvendedor.NomeGerente, 0), 50, ' ');
			--CODIGO
			Vslinha := Vslinha || Rpad(nvl(Vtvendedor.NroGerente, 0), 11, ' ');
        ...
        for vEmpresas in (Select x.Sequencia as CodEmpresa
			...
			For Vtvenda In (--NUMERODF, SERIEDF, CODIGOPROD, DESCPRODUTO, QUANTIDADE, BONIFICACAO, VLRUNITARIO, VLRBRUTO, VLRLIQUIDO, TIPONOTAFISCAL
				SELECT
				   ...
				   /* Tirado o valor de desconto do valor bruto, conforme solicitado » Alterado por Hilson Santos em 01/12/2022 */
				   ...
				   CASE
				   WHEN v.codgeraloper in (201, 202, 225, 228, 235, 307, 313, 314, 575, 598, 701)
				   THEN '1'
  ...
  Begin
    If Psversaolayout = '4' Or Psversaolayout = '04' Or
      ...
      For Vtproduto In (Select a.Seqproduto As Codinternoproduto,
        ...
        --PREÇO CADASTRADO UNITÁRIO DO PRODUTO PARA VENDA
        Begin
        ...
        --PREÇO CADASTRADO UNITÁRIO DO PRODUTO PARA COMPRA
		    /*
        Begin
        ...
        */
		...
		--PREÇO CADASTRADO UNITÁRIO DO PRODUTO PARA COMPRA
        Vnvlrcustocompra := 0;
        Vnvlrcustocompra := Round(Fc_Cmdiacustobruto(Vtproduto.Codinternoproduto,Vtproduto.Nroempresa),2) * 100;
